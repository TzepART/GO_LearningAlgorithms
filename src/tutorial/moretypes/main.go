package main

import "tutorial/moretypes/packages"

func main()  {
	step24()
}

//1. Pointers
func step1()  {
	moretypes.PointerExample1()
}

//2. Struct
func step2()  {
	moretypes.StructExample2()
}

//3. Struct Fields
func step3()  {
	moretypes.StructExample3()
}

//4. Pointers to structs
func step4()  {
	moretypes.StructExample4()
}

//5. Struct Literals
func step5()  {
	moretypes.StructExample5()
}

//6. Arrays
func step6()  {
	moretypes.ArrayExample6()
}

//7. Slices form array
func step7()  {
	moretypes.SliceExample7()
}

//8. Slices are like references to arrays
func step8()  {
	moretypes.SliceExample8()
}

//9. Slice literals
func step9()  {
	moretypes.SliceExample9()
}

//10. Slice defaults
func step10()  {
	moretypes.SliceExample10()
}

//11. Slice length and capacity
func step11()  {
	moretypes.SliceExample11()
}

//12. Nil slices
func step12()  {
	moretypes.NilExample12()
}

//13. Creating a slice with make
func step13()  {
	moretypes.SliceExample13()
}

//14. Slices of slices
func step14()  {
	moretypes.SliceExample14()
}

//15. Appending to a slice
func step15()  {
	moretypes.SliceExample15()
}

//16. The range form of the for loop iterates over a slice or map.
func step16()  {
	moretypes.RangeExample16()
}

//17. Range continued.
func step17()  {
	moretypes.RangeExample17()
}

//18. Exercise: Slices
func step18()  {
	moretypes.ExersiseSlice18()
}

//19. Maps
func step19()  {
	moretypes.MapExample19()
}

//20. Map literals
func step20()  {
	moretypes.MapExample20()
}

//21. Map literals continued
func step21()  {
	moretypes.MapExample21()
}

//22. Mutating Maps
func step22()  {
	moretypes.MapExample22()
}

//23. Exercise: Maps
func step23()  {
	moretypes.ExersiseMaps23()
}

//24. Function values
func step24()  {
	moretypes.FunctionExample24()
}

//25.
//func step25()  {
//	moretypes.Example25()
//}
//
//26.
//func step26()  {
//	moretypes.Example26()
//}
